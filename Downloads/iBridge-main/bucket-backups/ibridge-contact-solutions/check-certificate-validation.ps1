# AWS Certificate Validation Checker Script
...
Write-Host "=== AWS Certificate Validation Checker ===" -ForegroundColor Cyan
...
if (-not $certificateArn) {
    Write-Host "No certificate found for $domainName. Please run the deployment script first." -ForegroundColor Red
    exit
}
...
if ($validationStatus -eq "PENDING_VALIDATION") {
    ...
    Write-Host "Your CloudFront distribution can now use HTTPS with this certificate." -ForegroundColor Green
    ...
    if (-not $usingDistribution) {
        ...
        Write-Host "Run the deployment script to create or update your CloudFront distribution." -ForegroundColor Yellow
    }
